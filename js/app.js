...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <div class="single-product ">
      ...
      <p>Total Rating :<strong>${product.rating.count}</strong></p>
      <button onclick="addToCart(${product.id},${product.price.toFixed(2)})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
...
// main price update function Math.round(total)
...
// set innerText function Math.round(value)
...
//grandTotal update function but i faild to add all.
const updateTotal = () => {
  const grandTotal =
    getInputValue("price") + setInnerText("delivery-charge") +
    updatePrice("total-tax");
  document.getElementById("total").innerText = grandTotal;
};
loadProducts();

//load id from FakeStoreApi 
...
//try to display details info in a card in website.
const displayDetails = (displays) => {
  console.log(displays)
  const display = document.getElementById('display-detais');
  const divCreate = document.createElement('div');
  divCreate.classList.add('card-body');
  divCreate.innerHTML = `
        <h5 class="card-title"><strong>Category :</strong> ${displays.category}</h5>
        <h6 class="card-subtitle mb-2 text-muted"><strong>Description :</strong>${displays.description}</h6>
        <p class="card-text">Id : ${displays.id}</p>
        <p class="card-text">Price : ${displays.price}</p>
`
